...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (form.password !== form.passwordConfirm) { setError("Passwörter stimmen nicht überein"); setSection("zugangsdaten"); return; }
    if (!form.name || !form.email || !form.password) { setError("Bitte Name, E-Mail und Passwort ausfüllen"); setSection("zugangsdaten"); return; }
    if (form.password.length < 8) { setError("Passwort muss mindestens 8 Zeichen haben"); setSection("zugangsdaten"); return; }
  ...
  return (
    <div className="min-h-screen max-w-lg mx-auto px-4 py-10 space-y-6">
      ...
      <p className="text-center text-sm text-[rgba(242,237,228,.4)]">
        Bereits Partner?{" "}
